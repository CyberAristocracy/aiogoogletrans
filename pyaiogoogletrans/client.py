...
from pyaiogoogletrans import urls, utils
from pyaiogoogletrans.gtoken import TokenAcquirer
from pyaiogoogletrans.constants import (
    DEFAULT_CLIENT_SERVICE_URLS,
    DEFAULT_FALLBACK_SERVICE_URLS,
    DEFAULT_USER_AGENT, LANGCODES, LANGUAGES, SPECIAL_CASES,
    DEFAULT_RAISE_EXCEPTION, DUMMY_DATA
)
from pyaiogoogletrans.models import Translated, Detected, TranslatedPart
...
class Translator():
    ...
    def __init__(self, service_urls=DEFAULT_CLIENT_SERVICE_URLS,
        ...
        if use_fallback:
            ...
            # стандартный способ работы: использовать определенные значения из приложения пользователя
    ...
    async def _async_translate(self, text: str, dest: str, src: str):
    ...
    async def _async_translate_legacy(self, text: str, dest, src, override):
    ...
    async def async_translate(self, text: str, dest='en', src='auto'):
        ...
        data, response = await self._async_translate(text, dest, src)
        token_found = False
    ...
    async def async_translate_legacy(self, text: str, dest='en', src='auto', **kwargs):
        dest = dest.lower().split('_', 1)[0]
        src = src.lower().split('_', 1)[0]
        ...
        if isinstance(text, list):
            result = []
            for item in text:
                translated = await self._async_translate_legacy(item, dest=dest, src=src, **kwargs)
                result.append(translated)
            return result

        origin = text
        data, response = await self._async_translate_legacy(text, dest, src)
    ...
    async def async_detect(self, text: str):
    ...
    async def async_detect_legacy(self, text, **kwargs):
        ...
        if isinstance(text, list):
            result = []
            for item in text:
                lang = await self.detect_legacy(item, **kwargs)
                result.append(lang)
            return result
    ...
    async def _translate(self, texts: list[str], dest='en', src='auto', **kwargs):
        tasks = [self.async_translate(text, dest=dest, src=src, **kwargs)
                 for text in texts]
        return await asyncio.gather(*tasks)

    def translate(self, texts: list[str] | str, dest='en', src='auto', **kwargs):
        loop = asyncio.get_event_loop()
        if isinstance(texts, str):
            return loop.run_until_complete(self.async_translate(texts, dest, src, **kwargs))
        results = loop.run_until_complete(
            self._translate(texts, dest, src, **kwargs))
        return results

    async def _translate_legacy(self, texts: list[str], dest='en', src='auto', **kwargs):
        tasks = [self.async_translate_legacy(
            text, dest=dest, src=src, **kwargs) for text in texts]
        return await asyncio.gather(*tasks)

    def translate_legacy(self, texts: list[str] | str, dest='en', src='auto', **kwargs):
        loop = asyncio.get_event_loop()
        if isinstance(texts, str):
            return loop.run_until_complete(self.async_translate_legacy(texts, dest, src, **kwargs))
        results = loop.run_until_complete(
            self._translate_legacy(texts, dest, src, **kwargs))
        return results
