...
class AsyncTranslator():
    ...
    async def translate_legacy(self, text, dest='en', src='auto', **kwargs):
        ...
        if isinstance(text, list):
            result = []
            for item in text:
                translated = await self.translate_legacy(item, dest=dest, src=src, **kwargs)
                result.append(translated)
            return result

        origin = text
        data, response = await self.translate_legacy(text, dest, src)
    ...
    async def _translations(self, texts: list[str], dest='en', src='auto', **kwargs):
        tasks = [self.translate(text, dest=dest, src=src, **kwargs) for text in texts]
        return await asyncio.gather(*tasks)
    
    def translations(self, texts: list[str], dest='en', src='auto', **kwargs):
        loop = asyncio.get_event_loop()
        results = loop.run_until_complete(self._translations(texts, dest, src, **kwargs))
        return results
    
    async def _translations_legacy(self, texts: list[str], dest='en', src='auto', **kwargs):
        tasks = [self.translate_legacy(text, dest=dest, src=src, **kwargs) for text in texts]
        return await asyncio.gather(*tasks)
    
    def translations_legacy(self, texts: list[str], dest='en', src='auto', **kwargs):
        loop = asyncio.get_event_loop()
        results = loop.run_until_complete(self._translations_legacy(texts, dest, src, **kwargs))
        return results